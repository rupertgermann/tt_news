...
function user_languageMenu($content)
{
    ...
    if ($GLOBALS['TSFE']->page['l18n_cfg'] == 1) { // = "Hide default Translation" is activated
        ...
        $flags[0] = ($tmpLang == 0 ? $pointer : '') . $GLOBALS['TSFE']->cObj->typolink($defaultflag['on'], array(
                'parameter' => $GLOBALS['TSFE']->id . ' _top',
                'additionalParams' => (!preg_match('/&L=[0-9]/',
                    $newsAddParams) ? $newsAddParams . '&L=0' : $newsAddParams)
            ));
    }
    ...
    if (is_array($flagimages)) {
        foreach ($flagimages as $fk => $fv) {
            if ($langArr[$fk]) {
                $flags[$fk] = ($tmpLang == $fk ? $pointer : '') . $GLOBALS['TSFE']->cObj->typolink($fv['on'], array(
                        'parameter' => $GLOBALS['TSFE']->id . ' _top',
                        'additionalParams' => (!preg_match('/&L=[0-9]/',
                            $newsAddParams) ? $newsAddParams . '&L=' . $fk : $newsAddParams)
                    ));
            } else {
                $flags[$fk] = $fv['off'];
            }
