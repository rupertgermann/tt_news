...
function user_substPageBrowser($markerArray, $conf)
{
    ...
    $results_at_a_time = GeneralUtility::intInRange($pObj->internal['results_at_a_time'], 1, 1000);
    $maxPages = GeneralUtility::intInRange($pObj->internal['maxPages'], 1, 100);
    $max = GeneralUtility::intInRange(ceil($count / $results_at_a_time), 1, $maxPages);
    $pointer = (int)$pointer;
    $links = [];
    ...
    if ($pObj->pi_alwaysPrev >= 0) {
        if ($pointer > 0) {
            $links[] = '
					<td nowrap="nowrap"><p>' . $pObj->pi_linkTP_keepPIvars($pObj->pi_getLL('pi_list_browseresults_prev', '< Previous'), ['pointer' => ($pointer - 1 ?: '')], $pObj->allowCaching) . '</p></td>';
    ...
    for ($a = 0;$a < $max;$a++) {
        $links[] = '
					<td' . ($pointer == $a ? $pObj->pi_classParam('browsebox-SCell') : '') . ' nowrap="nowrap"><p>' . $pObj->pi_linkTP_keepPIvars(trim($pObj->pi_getLL('pi_list_browseresults_page', 'Page') . ' ' . ($a + 1)), ['pointer' => ($a ?: '')], $pObj->allowCaching) . '</p></td>';
    }
    if ($pointer < ceil($count / $results_at_a_time) - 1) {
        $links[] = '
					<td nowrap="nowrap"><p>' . $pObj->pi_linkTP_keepPIvars($pObj->pi_getLL('pi_list_browseresults_next', 'Next >'), ['pointer' => $pointer + 1], $pObj->allowCaching) . '</p></td>';
    }
    ...
    (
        ...
        ($pObj->internal['res_count'] ?
            sprintf(
                str_replace('###SPAN_BEGIN###', '<span' . $pObj->pi_classParam('browsebox-strong') . '>', (string)$pObj->pi_getLL('pi_list_browseresults_displays', 'Displaying results ###SPAN_BEGIN###%s to %s</span> out of ###SPAN_BEGIN###%s</span>')),
...
function user_substPageBrowser2($markerArray, $conf)
{
    ...
    if ($newsCount > $begin_at + $pObj->config['limit']) {
        ...
        $next = (int)($next / $pObj->config['limit']);
        $markerArray['###LINK_NEXT###'] = $pObj->pi_linkTP_keepPIvars($pObj->pi_getLL('pi_list_browseresults_next', 'Next >'), ['pointer' => $next], $pObj->allowCaching);
    } else {
    ...
    if ($begin_at) {
        ...
        $prev = (int)($prev / $pObj->config['limit']);
        $markerArray['###LINK_PREV###'] = $pObj->pi_linkTP_keepPIvars($pObj->pi_getLL('pi_list_browseresults_prev', '< Previous'), ['pointer' => $prev], $pObj->allowCaching) . ' ';
    } else {
    ...
    for ($i = $firstPage; $i < $lastPage; $i++) {
        if (($begin_at >= $i * $pObj->config['limit']) && ($begin_at < $i * $pObj->config['limit'] + $pObj->config['limit'])) {
            ...
            $markerArray['###BROWSE_LINKS###'] .= $pObj->pi_linkTP_keepPIvars($pObj->local_cObj->stdWrap($item, $pObj->conf['pageBrowser.']['page_stdWrap.']) . ' ', ['pointer' => $i], $pObj->allowCaching) . ' ';
        }
    }
