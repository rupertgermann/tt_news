		...
		if ($tmpcat !== false) {
			...
			while (($row = $this->db->sql_fetch_assoc($res))) {
				...
				if ($this->conf['displaySubCategories'] && $this->config['useSubCategories']) {
					$subCategories = array();
					$subcats = implode(',', array_unique(explode(',', \RG\TtNews\Lib\tx_ttnews_div::getSubCategories($rows[0]['uid'], $addWhere))));
	...
	function initCategoryVars() {
		...
		if ($this->piVars['cat']) {
			...
			if ($this->config['useSubCategories'] && $this->config['catSelection']) {
				...
				$subcats = \RG\TtNews\Lib\tx_ttnews_div::getSubCategories($this->config['catSelection'], $addWhere);
		...
		if ($this->config['useSubCategories'] && $this->catExclusive) {
			$subcats = \RG\TtNews\Lib\tx_ttnews_div::getSubCategories($this->catExclusive, $addWhere);
