...
class tx_ttnews_module1 extends \TYPO3\CMS\Backend\Module\BaseScriptClass
{
    ...
    function init()
    {
        ...
        $pidForNewArticles = intval($this->TSprop['list.']['pidForNewArticles']);
        $this->newArticlePid = ($pidForNewArticles ? $pidForNewArticles : $this->id);

        $this->script = 'mod.php?M=web_txttnewsM1';

        if ($this->TSprop['list.']['fList']) {
            $this->fieldList = $this->TSprop['list.']['fList'];
        }
...
//					');
    ...
    function initCategories()
    {
        ...
        $this->catlistWhere = \WMDB\TtNews\Lib\tx_ttnews_div::getCatlistWhere($this->confArr['useStoragePid'] ? $this->storagePid : -1);
    }
