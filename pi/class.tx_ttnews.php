...
class tx_ttnews extends \TYPO3\CMS\Frontend\Plugin\AbstractPlugin
{
    ...
    function displayList($excludeUids = '0')
    {
        ...
        if (!$this->conf['emptyArchListAtStart'] && ($this->arcExclusive > 0 && !$this->piVars['pS'] && $theCode != 'SEARCH')) {
        ...
        if ($row['minval'] || $row['maxval']) {
            ...
            if ($cachedPeriodAccum != '') {
                ...
                if ($this->cache_amenuPeriods && count($periodAccum)) {
                    ...
                    $this->cache->set($storeKey, serialize($periodAccum), array(__FUNCTION__));
                }
    ...
    function getCategories($uid, $getAll = false)
    {
        ...
        if ($tmpcat !== false) {
            ...
            if ($this->cache_categories && is_array($categories)) {
                $this->cache->set($hash, serialize($categories), array(__FUNCTION__));
            }
    ...
    function initCaching()
    {
        ...
        $this->cache = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('TYPO3\\CMS\\Core\\Cache\\CacheManager')->getCache('tt_news_cache');
        $this->cache->getBackend()->setDefaultLifetime(intval($lifetime, 10));
    }
